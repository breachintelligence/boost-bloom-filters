...
namespace boost {
  namespace bloom_filters {
	...
	((uint32_t*)out)[3] = h4;
      ...
      void murmurhash3_x64_128(const void *const __restrict key, const size_t len,
			       const size_t seed, const void *__restrict out )
      {
	const uint8_t *const __restrict data = static_cast<const uint8_t*>(key);
	const int nblocks = len / 16;
	...
	const uint64_t *const __restrict blocks = reinterpret_cast<const uint64_t *>(data);
	...
	const uint8_t *const __restrict tail = static_cast<const uint8_t*>(data + nblocks*16);
